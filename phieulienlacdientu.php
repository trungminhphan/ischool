...
$danhhieu_hk1='';$danhhieu_hk2='';$danhhieu_cn='';$id_danhsachlop = '';
		...
		if($mamonhoc == 'THEDUC' || $mamonhoc ==='AMNHAC' || $mamonhoc == 'MYTHUAT'){
			...
			echo '<td class="border_right">'.$mh['tenmonhoc'].'</td>';
				...
				foreach($phieulienlac_list as $pll){
					...
					} else if($diem_thi_cd_hk1 =='Đ' && $diem_d_hk1 > 0 && round($diem_d_hk1/($diem_d_hk1 + $diem_cd_hk1), 2) >= 0.65) {
						$trungbinh1 = 'Đ';$trungbinh_d_hk1++;
					...
					} else if($diem_thi_cd_hk2 == 'Đ' && $diem_d_hk2 > 0 && round($diem_d_hk2/($diem_d_hk2 + $diem_cd_hk2), 2) >= 0.65) {
						$trungbinh2 = 'Đ';$trungbinh_d_hk2++;
...
<table  width="100%" border="1" cellpadding="20" id="bangdiem_1200" style="margin-top:3px;">
	<tr style="height:100px;">
		<td width="30%" style="vertical-align:top;">
			<center><b>Ý KIẾN PHHS</b>
			<?php if($users->is_student()) : ?>
				<a href="ykienphhs" onclick="return false;" class="ykien"><span class="mif-plus"></span></a>
			<?php endif; ?>
			</center>
			<div>
				<ol class="ykienphhs_list">
					...
					if(isset($pll['ykienphhs'])){
						foreach ($pll['ykienphhs'] as $k=>$phhs) {
							echo '<li>'.$phhs['noidung'].'<span class="tag">'.date("d/m/Y h:i", $phhs['ngaycapnhat']->sec).'&nbsp;<a href="xoaykien.php?id_danhsachlop='.$id_danhsachlop.'&ykien=ykienphhs&id='.$phhs['_id'].'&_token='.$csrf->getToken().'" class="xoaykien" onclick="return false;"><span class="mif-bin"></span></a></span></li>';
						}
					}
					?>
				</ol>				
				...
				<ol class="ykiengvcn_list">
					...
					if(isset($pll['ykiengvcn'])){
						foreach ($pll['ykiengvcn'] as $k=>$gvcn) {
							echo '<li>'.$gvcn['noidung'].'<span class="tag">'.date("d/m/Y h:i", $gvcn['ngaycapnhat']->sec).'</span></li>';
						}
					}
					?>
				</ol>				
...
<div data-role="dialog" id="dialog_ykien" class="info padding20" data-close-button="false" data-overlay="true" style="min-width:400px;">
	...
	<div class="grid">
		...
		<?php if($users->is_student()): ?>
		<div class="row cells12">
			<div class="cell colspan4">Mã xác nhận</div>
			<div class="cell colspan8" class="input-control text">
				<input type="password" name="maxacnhanphuhuynh" id="maxacnhanphuhuynh" value="" />
			</div>
		</div>
		<?php endif; ?>
		<div class="row cells12">
...
<div data-role="dialog" id="dialog_xoaykien" class="alert padding20" data-close-button="false" data-overlay="true" style="min-width:400px;">
	...
	<div class="grid">
		...
		<div class="row cells12">
			<div class="cell colspan4">Mã xác nhận</div>
			<div class="cell colspan8" class="input-control text">
				<input type="password" name="maxacnhanphuhuynh_del" id="maxacnhanphuhuynh_del" value="" />
			</div>
		</div>
		<?php endif; ?>
		<div class="row cells12">
