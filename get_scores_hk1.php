	...
	foreach ($danhsachlop_list as $ds) {
		...
		foreach ($list_monhoc as $mmh) {
			...
			if($mamonhoc=='THEDUC' || $mamonhoc=='AMNHAC' || $mamonhoc=='MYTHUAT'){
				...
				} else if($diem_thi_cd =='Đ' && $diem_d > 0 && round($diem_d/($diem_d + $diem_cd), 2) >= 0.65) {
					$trungbinhmon = 'Đ';$trungbinh_d++;
		...
		if(isset($ds['danhgia_hocky1'])){
			$hanhkiem = isset($ds['danhgia_hocky1']['hanhkiem']) ? $ds['danhgia_hocky1']['hanhkiem'] : '';
		} 
		//Xep loai hoc luc
		...
		if($diemtrungbinh){
			array_push($ranges_hk1, $diemxephang);
		}
		$i++;
	}
?>