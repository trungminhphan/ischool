...
<h1><a href="index.html" class="nav-button transform"><span></span></a>&nbsp;Danh sách lớp theo danh hiệu</h1>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Danh hiệu:
<div class="input-control select">
	<select name="danhhieu" id="danhhieu" class="select2">
		<?php
		foreach ($arr_danhhieu as $key => $value) {
			echo '<option value="'.$key.'"'.($danhhieu_list == $key ? ' selected' : '').'>'.$value.'</option>';
		}
		?>
	</select>
</div>
...
<?php if($danhsachlop_list && $danhsachlop_list->count() > 0) : ?>
	...
	<tr>
		...
		<td align="center">
			<h3>DANH SÁCH HỌC SINH THEO DANH HIỆU</h3>
		</td>
	</tr>
	<tr>
...
<?php
	...
	foreach ($arr_hocsinh as $ds) {
		...
		$hocky = 'hocky2';
		if(isset($ds['danhgia_'.$hocky]) && $ds['danhgia_'.$hocky]){
			$hanhkiem = isset($ds['danhgia_'.$hocky]['hanhkiem']) ? $ds['danhgia_'.$hocky]['hanhkiem'] : '';
			if($hanhkiem == 'T') $count_hk_tot++;
			else if($hanhkiem == 'K') $count_hk_kha++;
			else if($hanhkiem == 'TB') $count_hk_tb++;
