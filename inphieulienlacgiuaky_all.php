...
if(isset($_GET['submit'])){
	if($id_namhoc && $id_lophoc){
		...
		$danhsachlop_list = $danhsachlop->get_danh_sach_lop_pll_giuaky();
	} else { $phieulienlac_list = ''; }
}
?>
<!DOCTYPE html>
<html>
			...
			echo '<td class="border_right">'.$mh['tenmonhoc'].'</td>';
				...
				foreach($phieulienlac_list as $pll){
					...
					if($diem_d_hk1 > 0 && $diem_cd_hk1==0){
						...
						$trungbinh1 = 'M';
					} else if($diem_cd_hk1 > 0 && round($diem_d_hk1/($diem_d_hk1 + $diem_cd_hk1), 2) < 0.65){
					...
					if($diem_d_hk2 > 0 && $diem_cd_hk2==0){
						...
						$trungbinh2 = 'M';
					} else if($diem_cd_hk2 > 0 && round($diem_d_hk2/($diem_d_hk2 + $diem_cd_hk2), 2) < 0.65){
...
?>
	<table width="100%" border="1" cellpadding="10" id="bangdiem" style="margin-top:5px;">
		<tr>
			<th>TỔNG KẾT</th>
		...
		<?php else: ?>
		<tr>
			<th><?php echo date("d/m/Y"); ?> (HK II)</th>
		...
		<!--<tr>
			<th><?php //echo date("d/m/Y"); ?> (Cả năm)</th>
...
<table  width="100%" border="1" cellpadding="20" id="bangdiem" style="margin-top:5px;">
				...
				<ol class="ykiengvcn_list">
					...
					if(isset($pll['ykiengvcn'])){
						foreach ($pll['ykiengvcn'] as $k=>$gvcn) {
							echo '<li>'.$gvcn['noidung'].'<span class="tag">'.date("d/m/Y h:i", $gvcn['ngaycapnhat']->sec).'</li>';
						}
					}
					?>
				</ol>
...
<?php endforeach; //end foreach danhsachlop ?>
</body>
</html>
